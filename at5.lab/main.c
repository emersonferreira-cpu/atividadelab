...
void ordenarPorDistancia(Entidade *jogador) {
    ...
    for (int i = 1; i < totalEntidades - 1; i++) {
        ...
        float distMenorQuad = (dx1 * dx1) + (dy1 * dy1);
        ...
        for (int j = i + 1; j < totalEntidades; j++) {
            ...
            float distQuad = (dx2 * dx2) + (dy2 * dy2);

            if (distQuad < distMenorQuad) {
                distMenorQuad = distQuad;
                indiceMenor = j;
            }
        }
        ...
        if (indiceMenor != i) {
            ...
            vetorEntidades[i] = vetorEntidades[indiceMenor];
            vetorEntidades[indiceMenor] = temp;
        }
    }
}
